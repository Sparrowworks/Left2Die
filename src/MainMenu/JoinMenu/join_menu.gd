...
func _on_join_button_pressed() -> void:
	...
	if ip_edit.text == "":
		var popup: MessagePopup = Messenger.create_popup(
			"Invalid IP", "The IP address cannot be empty."
		)
		add_child(popup)
		return
	...
	if error != OK:
		var popup: MessagePopup = Messenger.create_popup(
			"Unknown error", "Unknown error has been returned with code: " + str(error)
		)
		add_child(popup)
		return
...
func _on_join_failed(quiet: bool) -> void:
	...
	if not quiet:
		var popup: MessagePopup = Messenger.create_popup(
			"Join Timeout", "Connection was terminated, because the game didn't respond."
		)
		add_child(popup)
